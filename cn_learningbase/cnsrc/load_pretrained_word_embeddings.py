...
class Glove:
    ...
    def _load(self, fn):
        ...
        emb = []
        word2VecModel = gensim.models.KeyedVectors.load_word2vec_format(fn, binary=True)
        for key in word2VecModel.wv.vocab.keys():
            word=key
            coefs = np.asarray(word2VecModel[key], dtype='float32')
            if self.dim:
