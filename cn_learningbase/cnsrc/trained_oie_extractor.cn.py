...
#coding=utf-8
from rnn.modelcn import load_pretrained_rnn
    ...
    def conll_with_prob(self, sent):
        ...
        for ((pred_ind, pred_word), labels) in self.model.predict_sentence(sent):
            ...
            for i,label_list in enumerate(labels):
                for j in range(0,min(2,len(label_list))):
                    label=label_list[j]
                    if label[0].startswith('P') and label[1]>maxProb:
                        maxProb=label[1]
                        Pflag=(i,j)
            if Pflag!=(-1,-1):
                li=Pflag[0]
                lj=Pflag[1]
                if lj==1:
                    labels[li][0]=labels[li][1]
                Aflag=-1
                AProb=0
                for i in range(0,li):
                    if len(labels[i])<2:
                        if labels[i][0][0].startswith('A'):
                            Aflag=-1
                            break
                        else:
                            continue
                    if labels[i][0][0].startswith('A'):
                        Aflag=-1
                        break
                    elif labels[i][1][0].startswith('A') and labels[i][1][1]>AProb:
                        AProb=labels[i][1][1]
                        Aflag=i
                if Aflag!=-1:
                    labels[Aflag][0]=labels[Aflag][1]
                Aflag=-1
                AProb=0
                for i in range(li+1,len(labels)):
                    if len(labels[i])<2:
                        if labels[i][0][0].startswith('A'):
                            Aflag=-1
                            break
            ...
            labels=[(label_list[0][0], label_list[0][1]) for label_list in labels]
            for (word_ind, ((label, prob), word)) in enumerate(zip(labels, sent)):
                ret+= "\t".join(map(str,
                                         [word_ind, word, pred_ind, label, prob]
                                     )) + '\n'
            # for (word_ind, (label_list, word)) in enumerate(zip(labels, sent)):
                
            #     ret+= "\t".join(map(str,
            #                              [word_ind, word, pred_ind]
            #                          ))+'\t'+" ".join([label[0]+','+str(label[1]) for label in label_list]) + '\n'
            ret += '\n'
        return ret
