...
from common.symbols import CN_POS_TAGS
from collections import defaultdict
from parsers.nlpir_wrapper import nlpir_parser as nlpir_ws
...
class RNN_model:
    ...
    def predict_sentence(self, sent):
        ...
        ret = []
        sent_str = " ".join(sent)
        sent_nospace=''.join(sent)

        # Extract predicates by looking at verbal POS

        preds = [(word['i'], word['word'])
                 for word
                 in nlpir_ws(sent_nospace)
        ...
        for ind, pred in preds:
            ...
            ret.append(((ind, pred),
                        [[(self.consolidate_label(label), float(prob)) for (label, prob) in label_list]
